...
class I18n {
  ...
  get(translationId) {
    ...
    if(translatedText && interpolationParameters && interpolationParameters.length > 0) {
      return this.interpolate(translatedText, interpolationParameters);
    }
  ...
  getTranslation(translationId, languageKey) {
    ...
    var translations = this.loadTranslations(langKey.toLowerCase());
  ...
  loadTranslations(locale) {
    ...
    if(!(/\{part\}/g).test(this.urlTemplate)) {
      ...
      return extend(this.translations[locale], this.load(url));
    }
    ...
    for(let prioritizedPart of prioritizedParts) {
      ...
      extend(this.translations[locale], this.load(url));
    }
...
class Part {
  ...
  load(locale, urlTemplate) {
    if (!this.translations[locale]) {
      let url = this.parseUrl(urlTemplate, locale);
      this.translations[locale] = require(url);
    }
    
    return this.translations[locale];
  }
  
  parseUrl(urlTemplate, locale) {
    return urlTemplate.replace(/\{part\}/g, this.name).replace(/\{lang\}/g, locale);
  }
  
  getTranslations(locale) {
    return locale ? this.translations[locale] : this.translations;
  }
